...
Module globalVar
        ...
        Try
            ...
            For Each row As DataRow In dtTasks.Rows
                Try
                    ...
                    Dim tasksUrl = Url + String.Format("{0}/projects/{1}/experiments/{2}/tasks/{3}/results/?page[size]=100", TeamsId, ProjectId, ExperimentId, taskId)
    ...
    Public Sub attachmenttabC(ByVal refToken As String)
        Try
            'readFromXML()
            'initSciNoteDB()
            'Dim url_ = Url
            ...
            For Each row As DataRow In dtTasks.Rows
                Try
                    ...
                    If taskId = 1876 Then
                        Dim abcd = 1
                    End If
                    Dim tasksUrl = Url + String.Format("{0}/projects/{1}/experiments/{2}/tasks/{3}/results?include=comments", TeamsId, ProjectId, ExperimentId, taskId)
                    Dim urip As Uri = New Uri(tasksUrl)
                    Dim responsesp As String = ""
                    responsesp = Woo_GetAllTeams(urip, refToken)
                    Dim datalistp As clsattchment = JsonConvert.DeserializeObject(Of clsattchment)(responsesp)
                    If datalistp.included.Count > 0 Then
                        For Each attri As includedc In datalistp.included
                            ...
                            Dim rrs As attrib = JsonConvert.DeserializeObject(Of attrib)(rr)
                            'rr = ""
                            Dim txt As String = rrs.text
                            'Dim file_name As String = rrs.file_name
                            'Dim url As String = rrs.url
                            If txt <> "" Then
                                Dim SQLinsertatt As String = String.Format("INSERT INTO [dbo].[resulttext]([t_id] ,[p_id] ,[e_id] ,[ta_id],result ) VALUES ('{0}' ,'{1}' ,'{2}' ,'{3}' ,'{4}')", TeamsId, ProjectId, ExperimentId, taskId, txt)
                                ExcuteNonQuery_SciNote(SQLinsertatt, True)
    ...
    Public Sub Main()
        ...
        If responses <> "" Then
            ...
            Next
        End If
        '--Update Projects By SAP No. not required
        'Dim sapServicedt As DataTable = getDataTableSAP(" Select CstmrCode,ContractId,Descriptio,U_Project from OCTR where U_Sync = 'No' and isnull(Descriptio,'') <>'' and isnull(U_Project,'')='' and 10 = 100")
        Dim sapServicedt As DataTable = getDataTableSAP(" Select CstmrCode,ContractId,Descriptio,U_Project from OCTR where U_Sync = 'Yes'")
        ...
        For Each rowt As DataRow In teamsDt.Rows
            ...
            Dim projectUrl = Url + String.Format("{0}/projects/?page[size]=100", teamsId)
            Dim urip As Uri = New Uri(projectUrl)
            Dim responsesp As String = ""
            ...
            For Each pData As datap In datalistp.data
                ...
                ' Dim archived As String = pData.archived
                Dim ff_ As New attrip
                ff_ = JObject.FromObject(data_).ToObject(Of attrip)
                Dim name = ff_.name, visibility = ff_.visibility, start_date = ff_.start_date, archived = ff_.archived, created_at = ff_.created_at, updated_at = ff_.updated_at
                Dim start_date_ = DateTime.Parse(start_date).ToString("yyyy-MM-ddTHH:mm:ss")
        ...
        '---GetExperiments 
        Dim projectsDt As DataTable = getDataTable("Select * from projects where ""DocEntry"" is not null order by t_id,s_id")
        ...
        For Each rowt As DataRow In projectsDt.Rows
            ...
            Dim experimentsUrl = Url + String.Format("{0}/projects/{1}/experiments/?page[size]=100", teamsId, projectsId)
        ...
        For Each rowt As DataRow In experimentsDt.Rows
            Try
                Dim teamsId = 4 'rowt("t_id")
                Dim projectsId = 38 'rowt("p_id")
                Dim experimentsId = rowt("s_id")
                Dim tasksUrl = Url + String.Format("{0}/projects/{1}/experiments/{2}/tasks/?page[size]=100", teamsId, projectsId, experimentsId)
                Dim urip As Uri = New Uri(tasksUrl)
                Dim responsesp As String = ""
                responsesp = Woo_GetAllTeams(urip, access_Token)
                Dim datalistp As clsProject = JsonConvert.DeserializeObject(Of clsProject)(responsesp)
        ...
        '    End Try
        'Next
        writeLog(String.Format("attachmenttabC "), False)
        Try
            attachmenttabC(access_Token)
        Catch ex As Exception

        End Try

        writeLog(String.Format("attachmenttabCEnd "), False)
        Try
        ...
        writeLog(String.Format("attachmenttab "), False)
        Dim dtAttach As DataTable = getDataTable("select * from [resultattachment]")
        writeLog(String.Format("resultattachment "), False)
        For Each row As DataRow In dtAttach.Rows
            Try
                ...
                Try
                    writeLog(String.Format("AttcUrl {0} | {1}", taskId, url), False)
                Catch ex As Exception
    ...
    Public Sub initSciNoteDB()
        Dim DBQuery = "Create Database " + Database
        ...
        Dim WOO_Order_resulttext = "CREATE TABLE  resulttext (id int IDENTITY(1,1) NOT NULL,[t_id] nvarchar(50) not null, p_id nvarchar(50) not null,e_id nvarchar(50) not null,ta_id nvarchar(50) not null,result nvarchar(max),Sync nvarchar(50),primary key(t_id,p_id,e_id,ta_id))"

        ExcuteNonQuery_SciNote(WOO_Order_resulttext)
    End Sub
    ...
    Private Function Woo_WriteAttachment(uri As Uri, ByVal fileName As String, ByVal RefToken As String, ByVal TaskId As String) As String
        ...
        request = WebRequest.Create(uri)
        Try
            writeLog(String.Format("Attc URL {0}  taskId{1}", uri.AbsoluteUri, TaskId), False)
        Catch ex As Exception
            writeLog(String.Format("Attc URL ex{0} ", ex.Message), False)
        End Try
