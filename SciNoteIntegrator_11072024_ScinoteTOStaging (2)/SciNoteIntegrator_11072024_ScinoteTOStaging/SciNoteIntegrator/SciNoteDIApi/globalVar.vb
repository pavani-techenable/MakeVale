...
Module globalVar
    ...
    Public Sub Main()
        Try
            ...
            'Create Activity 
            Dim strtaskdata_ As String = "Select t0.*,t1.DocEntry,t2.CardCode   from tasks t0 inner join experiments t1 on t1.t_id = t0.t_id and t0.p_id =t1.p_id and t0.e_id =t1.s_id inner join teams t2 on t2.s_id =t0.t_id   where 
t1.DocEntry is not null  and t0.DocEntry is Null"

            Dim strtaskdata As String = "Select  Distinct t0.t_id , t0.p_id, t0.e_id   from tasks t0 inner join experiments t1 on t1.t_id = t0.t_id and t0.p_id =t1.p_id and t0.e_id =t1.s_id inner join teams t2 on t2.s_id =t0.t_id   where 
t1.DocEntry is not null  and t0.DocEntry is Null"
            Dim dt_taskData As New DataTable
            dt_taskData = getDataTable(strtaskdata)
            For Each rw As DataRow In dt_taskData.Rows

                Try
                    Dim t_id = rw("t_id")
                    Dim p_id = rw("p_id")
                    Dim e_id = rw("e_id")
                    writeLog(String.Format("Started loop in activity creation t_id = {0} , p_id = {1} ,e_id = {2} ", t_id, p_id, e_id), True)
                    Dim strtaskdata_1 As String = String.Format("Select  Distinct t0.t_id , t0.p_id, t0.e_id   from tasks t0 inner join experiments t1 on t1.t_id = t0.t_id and t0.p_id =t1.p_id and t0.e_id =t1.s_id inner join teams t2 on t2.s_id =t0.t_id   where 
t1.DocEntry is not null  and t0.DocEntry is Null and t0.t_id ={0} and t0.p_id={1} and t0.e_id={2}", t_id, p_id, e_id)
                    Dim dt_taskData_ As New DataTable
                    dt_taskData_ = getDataTable(strtaskdata_1)
                    objCompany.StartTransaction()
                    For Each row As DataRow In dt_taskData_.Rows
                        Try
                            'writeLog(String.Format("Started loop in activity creation activity = {0}", row("s_id")), True)
                            writeLog(String.Format("Started loop in activity creation activity = {0}", row("e_id")), True)
                            ...
                            'writeLog(String.Format("Sap loop in activity creation {0}", row("s_id")), True)
                            writeLog(String.Format("Sap loop in activity creation {0}", row("e_id")), True)
                            ...
                            If errors_.Count = 0 And DocEntry <> 0 Then
                                ...
                                ExcuteNonQuerySciNote(updateProject)
                            End If
                            If Not (objCompany.InTransaction()) Then
                                objCompany.EndTransaction(SAPbobsCOM.BoWfTransOpt.wf_RollBack)
                            End If
                        Catch ex As Exception
                            'writeLog(String.Format("Exception loop in activity creation s_id = {0} " + ex.Message, row("s_id")), True)
                            writeLog(String.Format("Exception loop in activity creation s_id = {0} " + ex.Message, row("e_id")), True)
                        End Try
                    Next
                    objCompany.EndTransaction(SAPbobsCOM.BoWfTransOpt.wf_Commit)
                Catch ex As Exception
...
t1.DocEntry is not null  and t0.DocEntry is not Null and T0.updatedes ='Yes'"
                ...
                For Each row As DataRow In dt_taskDataup.Rows
                    Try
                        ...
                        If errors_.Count = 0 And DocEntry <> 0 Then
                            Dim updateProject = String.Format("Update tasks set updatedes = 'No' where DocEntry = '{0}'", DocEntry.ToString, taskId)
                            ExcuteNonQuerySciNote(updateProject)
                        End If
                    Catch ex As Exception
            ...
            For Each row As DataRow In dt_linkSer.Rows
                Try
                    ...
                    objDiOperations.updateServiceByActivity(ActivityId, ServiceId, rw, taskId, errors_, DocEntry)
                    If errors_.Count = 0 And DocEntry <> 0 Then
                        Dim updateProject = String.Format("Update tasks set Sync = 'Yes' where s_id = '{0}'", taskId)
                        ExcuteNonQuerySciNote(updateProject)
                    End If
                Catch ex As Exception
